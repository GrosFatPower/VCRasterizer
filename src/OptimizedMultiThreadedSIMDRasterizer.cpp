// OptimizedMultiThreadedSIMDRasterizer.cpp - Impl�mentation compl�te
  ...
  : Renderer(w, h)
{
  ...
  _NumThreads = std::min(_NumThreads, 16); // Limiter � 16 threads max
  ...
  for (int ty = 0; ty < _TileCountY; ++ty)
  {
    for (int tx = 0; tx < _TileCountX; ++tx)
    {
      ...
      tileData.triangles.reserve(1000); // R�server de l'espace
    }
  }

  // Initialisation des donn�es thread-local
...
void OptimizedMultiThreadedSIMDRasterizer::TransformTrianglesVectorized(const glm::mat4& mvp)
{
  ...
  // Matrice transpos�e pour vectorisation optimale
...
#pragma omp parallel for
  for (int triIndex = 0; triIndex < triangleCount; ++triIndex) {
    ...
    for (int v = 0; v < 3; ++v) {
      ...
      // Conversion vers coordonn�es �cran
      if (clipSpace.w > 0.0f) {
...
void OptimizedMultiThreadedSIMDRasterizer::HierarchicalBinning()
{
  ...
  // Binning hi�rarchique avec frustum culling
#pragma omp parallel for
  for (int triIndex = 0; triIndex < _Transformed.size(); ++triIndex) {
    ...
    // Ajout du triangle aux tuiles concern�es
...
void OptimizedMultiThreadedSIMDRasterizer::RenderTrianglesMultiThreaded()
{
  ...
  // D�marrage du rendu multi-thread�
  {
  ...
  const Tile& tile, ThreadLocalData* localData)
{
  ...
  for (int blockY = startY; blockY < endY; blockY += 4) {
    for (int blockX = startX; blockX < endX; blockX += 8) {
      ...
      // Test de visibilit� rapide du bloc
...
{
  // Coordonn�es des 8 pixels avec offset 0.5 pour le centre du pixel
  ...
  // Test de signes (>= 0 pour �tre � l'int�rieur)
  ...
  const float* depths, const __m256i& mask, uint32_t color, ThreadLocalData* localData)
{
  ...
  // Mise � jour conditionnelle des profondeurs
...
{
  // Test rapide de visibilit� du bloc entier
  ...
  // Si au moins un coin est � l'int�rieur, le bloc est potentiellement visible
  for (int i = 0; i < 4; ++i) {
...
void OptimizedMultiThreadedSIMDRasterizer::SetupTriangleDataOptimized(TransformedTriangle& tri)
{
  ...
  // Calcul de l'aire sign�e du triangle
  tri.area = tri.edgeC[0] + tri.edgeC[1] + tri.edgeC[2];

  // V�rification de validit� du triangle
  ...
  // Pr�-calcul des inverses de profondeur pour interpolation perspective-correcte
  tri.invDepths[0] = (v0.w != 0.0f) ? 1.0f / v0.w : 1.0f;
  tri.invDepths[1] = (v1.w != 0.0f) ? 1.0f / v1.w : 1.0f;
  tri.invDepths[2] = (v2.w != 0.0f) ? 1.0f / v2.w : 1.0f;
}

// M�thodes additionnelles pour compl�ter l'interface Renderer
...
float OptimizedMultiThreadedSIMDRasterizer::InterpolateDepth1x_InverseZ(float x, float y, const TransformedTriangle& tri)
{
  ...
  // Interpolation lin�aire de la profondeur Z (pour le Z-buffer)
...
void OptimizedMultiThreadedSIMDRasterizer::UpdateZBuffer8x(int pixelIndex, const float* depths, const __m256i& mask, uint32_t color)
{
  ...
  // Mise � jour conditionnelle des couleurs
  // Note: Pour 8 pixels, nous devons traiter les couleurs individuellement ou utiliser des techniques plus avanc�es
...
// M�thodes de debugging et profiling
...
// Gestion des diff�rents modes de rendu
...
// Validation de la coh�rence des r�sultats
